...
ESX = nil
local TigerDragonName = GetCurrentResourceName()

Citizen.CreateThread(function()
    while ESX == nil do
        TriggerEvent('exs:getSharedObject', function(obj) ESX = obj end)
        Citizen.Wait(0)
    end
    PlayerData = ESX.GetPlayerData()
end)

RegisterNetEvent('esx:playerLoaded') -- register event
AddEventHandler( 
    'esx:playerLoaded', function(player) -- add evenhandler to player
        PlayerData = player
end)
...
AddEventHandler(
    'esx:setjob', function(job)
        PlayerData.job = job
end)



local font = RegisterFontFile'font4thai'
local press = false

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        for k,v in pairs(Config['lacation']) do
            if GetDistanceBetweenCoords(
                v.coords, GetEntityCoords(PlayerPedId)
            ) < 1.5 then
                if press == false then
                    DrawTxtmaxezText(0.96, 0.7, 1, 0.7, 0.55,
                    'กด ~o~[E]~s~ เล่นเสือมังกร', 255, 255, 255, 255)
                    if IsControlJustReleased(0, Keys[Config['button']]) then
                        press = true
                        print('jsut press')
                        print(press)
                        startanime()
                        startminigame()
                        -- ต้องให้รันเป็นรอบๆ startminigame
                        -- minigame ต้องรันรอบๆตามเซิฟเวอร์
                    end
                end
            end
        end
...
-- [[ พวก cooldown ในการที่ใฟ้โต้ะรันแต่ละรอบทำยังไง ไม่ได้รันตลอดที่มี client trigger]]--
